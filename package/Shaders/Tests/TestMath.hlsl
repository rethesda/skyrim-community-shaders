	...
	[numthreads(1, 1, 1)] void TestEpsilonConstants()
{
	...
	// EPSILON_SKIN_ALBEDO should be 0.001f
	ASSERT(IsTrue, EPSILON_SKIN_ALBEDO > 0.0f);
	ASSERT(IsTrue, EPSILON_SKIN_ALBEDO < 0.01f);
	ASSERT(AreEqual, EPSILON_SKIN_ALBEDO, 0.001f);

	// Verify ordering: DIVISION < DOT_CLAMP < SSS_ALBEDO = SKIN_ALBEDO
	ASSERT(IsTrue, EPSILON_DIVISION < EPSILON_DOT_CLAMP);
	ASSERT(IsTrue, EPSILON_DOT_CLAMP < EPSILON_SSS_ALBEDO);
	ASSERT(AreEqual, EPSILON_SSS_ALBEDO, EPSILON_SKIN_ALBEDO);
}
